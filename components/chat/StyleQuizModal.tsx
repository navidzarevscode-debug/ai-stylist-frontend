...
const STEPS: Step[] = [
  ...
  {
    key: "occasion",
    question: "توی مناسبت خاصی قراره پوشیده بشه ؟",
    options: [
      { value: "yes", label: "بله" },
      { value: "no", label: "خیر" },
...
export function detectKnownAnswers(message: string): Partial<StyleQuizAnswers> {
  ...
  // قد - دنبال عدد قبل یا بعد از کلمه "قد" یا "سانت" می‌گرده
  const heightMatch = text.match(/(?:قد[م]?[^\d]{0,5})?(\d{2,3})\s*(?:سانت|cm)?/);
  if (heightMatch) {
    const h = parseInt(heightMatch[1], 10);
    if (h >= 140 && h < 160) known.height = "150-160";
    else if (h >= 160 && h < 170) known.height = "160-170";
    else if (h >= 170 && h < 180) known.height = "170-180";
    else if (h >= 180 && h <= 220) known.height = "180+";
  }

  // وزن - دنبال عدد قبل یا بعد از کلمه "وزن" یا "کیلو" می‌گرده
  const weightMatch = text.match(/(?:وزن[م]?[^\d]{0,5})?(\d{2,3})\s*(?:کیلو|کیلوگرم|kg)/);
  if (weightMatch) {
    const w = parseInt(weightMatch[1], 10);
    if (w >= 30 && w < 50) known.weight = "30-50";
    else if (w >= 50 && w < 70) known.weight = "50-70";
    else if (w >= 70 && w < 90) known.weight = "70-90";
    else if (w >= 90 && w <= 200) known.weight = "90+";
  }
